...
func enter() -> void:
	...
	if not state_owner.animated_sprite.frame_changed.is_connected(_on_frame_changed):
		state_owner.animated_sprite.frame_changed.connect(_on_frame_changed)
...
func exit() -> void:
	...
	if state_owner.animated_sprite.frame_changed.is_connected(_on_frame_changed):
		state_owner.animated_sprite.frame_changed.disconnect(_on_frame_changed)
...
func pop_attack() -> ATTACK_TYPE:
	"""
	Pop the Attack Input (Light/Heavy) from the state_owner input buffer 
	and
	Push said Attack Input to the combo specific buffer
	"""
	var popped_attack: ATTACK_TYPE = ATTACK_TYPE.UNKNOWN
	var _foo = state_owner.attack_input_buffer
	
	if not state_owner.attack_input_buffer.is_empty():
		popped_attack = state_owner.attack_input_buffer.pop_back()
...
func _on_frame_changed():
	var current_frame_index: int = state_owner.animated_sprite.frame 
	var total_frames: int = state_owner.animated_sprite.sprite_frames.get_frame_count(state_owner.animated_sprite.animation)
	
	if current_frame_index == total_frames - 1:
		_on_last_frame(state_owner.animated_sprite)
	
	
func _on_last_frame(animation: AnimatedSprite2D): 
	"""
	Called when the state_owner reaches the last frame of the animation
	Args:
