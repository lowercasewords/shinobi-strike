...
func enter():
	...
	if not state_owner.state_machine.current_state is WallJumpVState:
		state_owner.animated_sprite.play("wall_jump_v_windup")
	elif abs(input_direction_h) == 1:
		jump_off_the_wall(-input_direction_h)

#func exit():
	#jump_off_the_wall(-state_owner.ninja_controller.get_input_direction_h())
...
func jump_off_the_wall(input_direction_h: float):
	...
	state_owner.animated_sprite.play("wall_jump_v")
	state_owner.velocity.x = JUMP_SPEED_INITIAL.x * -input_direction_h
	state_owner.velocity.y = JUMP_SPEED_INITIAL.y

func _on_animation_finished():
	var input_direction_h = sidewalls_collision_direction()
	if state_owner.animated_sprite.animation == "wall_jump_v_windup" and abs(input_direction_h) == 1:
		jump_off_the_wall(-input_direction_h)
